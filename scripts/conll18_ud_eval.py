...
ID, FORM, LEMMA, PREDICATE, PSENSE, UPOS, XPOS, FEATS, HEAD, DEPREL, DEPS, MISC = range(12)
...
def evaluate(gold_ud, system_ud):
    ...
    return {
        ...
        "PREDICATE": alignment_score(alignment, lambda w, _: w.columns[PREDICATE]),
        "PSENSE": alignment_score(alignment, lambda w, _: w.columns[PSENSE]),
...
def main():
    ...
    if not args.verbose and not args.counts:
        ...
        print("-----------+-----------+-----------+-----------+-----------")
        for metric in["Tokens", "Sentences", "Words", "PREDICATE", "PSENSE", "UPOS", "XPOS", "UFeats", "AllTags", "Lemmas", "UAS", "LAS", "CLAS", "MLAS", "BLEX"]:
            if args.counts:
