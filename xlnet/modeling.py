...
def transformer_xl(inp_k, n_token, n_layer, d_model, n_head,
  ...
  with tf.variable_scope(scope):
    ...
    for i in range(n_layer):
      ...
      hidden_states.append((output_h, output_g) if inp_q is not None else output_h)
    ...
    if inp_q is not None:
        hidden_states = tuple(h for hs in hidden_states for h in hs)
    else:
        hidden_states = tuple(hs for hs in hidden_states)
