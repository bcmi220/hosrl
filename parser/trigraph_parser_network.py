...
class TriGraphParserNetwork(BaseNetwork):
  ...
  def build_graph(self, input_network_outputs={}, reuse=True, debug=False, nornn=False):
    """"""
    print("Building TriGraphParserNetwork...")
    #pdb.set_trace()
    ...
    token_weights2D = tf.expand_dims(token_weights, axis=-1) * tf.expand_dims(token_weights, axis=-2)
    # as our three dimension a b c, is a->b to deciding, so all binary potential should not contain root
    token_weights4D = tf.cast(tf.expand_dims(token_weights2D, axis=-3) * tf.expand_dims(tf.expand_dims(token_weights, axis=-1),axis=-1),dtype=tf.float32)
            ...
            with tf.variable_scope('Unlabeled'):
              if self.layer_mask(head_vocab):
                unlabeled_outputs = head_vocab.get_bilinear_discriminator(
                  layer,
                  token_weights=token_weights3D,
                  reuse=reuse, debug=debug, token_weights4D=token_weights4D, prev_output=labeled_outputs)
              else:
                unlabeled_outputs = head_vocab.get_bilinear_discriminator(
                  layer,
                  token_weights=token_weights3D,
                  reuse=reuse, debug=debug, prev_output=labeled_outputs)
  ...
  @property
  def label_end2end(self):
    try:
      return self._config.getboolean(self, 'label_end2end')
    except:
      return False