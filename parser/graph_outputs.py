...
class GraphOutputs(object):
	...
	def probs_to_preds(self, probabilities, lengths, force_MST=False, get_argmax=False):
		...
		if 'predicate' in probabilities:
			predicate_probs = probabilities['predicate']
			predicate_preds = np.argmax(predicate_probs, axis=-1)
			predictions['predicate'] = predicate_preds
		if 'psense' in probabilities:
			psense_probs = probabilities['psense']
			psense_preds = np.argmax(psense_probs, axis=-1)
			predictions['psense'] = psense_preds
	...
	def print_current_predictions(self):
		...
		fields = ['form', 'lemma', 'predicate', 'psense', 'upos', 'xpos', 'frame', 'dephead', 'deprel', 'semrel', 'attr']
		for i in order:
			j = 1
			token = []
