...
from parser.neural.optimizers import AdamOptimizer, AMSGradOptimizer, AdamWeightDecayOptimizer
...
class BaseNetwork(object):
	...
	#=============================================================
	def __init__(self, input_networks=set(), config=None, is_eval=False):
		""""""
		#pdb.set_trace()
		self._config = config
		...
		for input_vocab_classname in self.input_vocab_classes:
			if input_vocab_classname in extant_vocabs:
				...
				vocab = VocabClass(config=config, is_eval=is_eval)
			...
			if 'Bert' in input_vocab_classname:
				...
				self.use_bert=False
			if 'Elmo' in input_vocab_classname:
				self.use_elmo=True
				self.elmovocab=self._input_vocabs[-1]
				self.pretrained_elmo=self.elmovocab.get_pretrained
			else:
				self.use_elmo=False
			if 'Albert' in input_vocab_classname:
				self.use_albert=True
				self.albertvocab=self._input_vocabs[-1]
				self.pretrained_albert=self.albertvocab.get_pretrained
			else:
				self.use_albert=False
			if 'Xlnet' in input_vocab_classname:
				self.use_xlnet=True
				self.xlnetvocab=self._input_vocabs[-1]
				self.pretrained_xlnet=self.xlnetvocab.get_pretrained
			else:
		...
		# print(self.throughput_vocab_classes)
	...
	def train(self, load=False, noscreen=False, debug=False, nornn=False):
		...
		for input_network in self.input_networks:
			with tf.variable_scope(input_network.classname, reuse=False):
				input_network_outputs[input_network] = (input_network, input_network.build_graph(reuse=True)[0])
		...
		# if self.use_bert and self.bertvocab.is_training:
		# 	the_vas = tf.trainable_variables()
		# elif self.use_albert and self.albertvocab.is_training:
		# 	the_vas = tf.trainable_variables()
		# elif self.use_xlnet and self.xlnetvocab.is_training:
		# 	the_vas = tf.trainable_variables()
		# else:
		the_vas = tf.trainable_variables(scope=self.classname)
		if self.use_elmo:
			elmo_variables= tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,scope='elmo')
			the_vas = the_vas + elmo_variables
		# if self.use_bert and self.bertvocab.is_training:
		# 	bert_variables= tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,scope='bert')
		# 	the_vas = the_vas + bert_variables
		# print('the_vas:', the_vas)

		#pdb.set_trace()
		update_step = tf.assign_add(self.global_step, 1)

		adam = AdamOptimizer(config=self._config)
		adam_op = adam.minimize(train_outputs.loss + regularization_loss, variables=the_vas) # returns the current step
		...
		amsgrad_op = amsgrad.minimize(train_outputs.loss + regularization_loss, variables=the_vas) # returns the current step
		...
		if self.use_adamW:
			...
			gvs = adamw.compute_gradients(train_outputs.loss, var_list=the_vas) 
		...
		if self.use_sgd_loss:
			...
			#sgd = tf.train.AdamOptimizer(learning_rate,adam.mu,adam.nu)
			gvs = sgd.compute_gradients(train_outputs.loss + regularization_loss, var_list=the_vas) 
			gradients, variables = zip(*gvs)
			gradients, _ = tf.clip_by_global_norm(gradients, adam.clip)
		...
		if self.save_model_after_improvement or self.save_model_after_training:
			...
			if self.use_elmo:
				elmo_variables=set(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope='elmo'))
				# print('elmo_variables:', elmo_variables)
				all_variables = all_variables | elmo_variables
			# if self.use_bert and self.bertvocab.is_training:
			# 	bert_variables=set(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope='bert'))
			# 	print('bert_variables:', bert_variables)
			# 	all_variables = all_variables | bert_variables
			# print('all_variables:', all_variables)
			non_save_variables = set(tf.get_collection('non_save_variables'))
			save_variables = all_variables - non_save_variables
			# print('save_variables:', save_variables)
		...
		with tf.Session(config=config) as sess:
			...
			if self.use_elmo and not self.pretrained_elmo:
				self.elmovocab.modelInit(sess)
			if self.use_albert and not self.pretrained_albert:
				self.albertvocab.modelInit(sess)
			if self.use_xlnet and not self.pretrained_xlnet:
				self.xlnetvocab.modelInit(sess)
								...
								if self.dev_mst:
									...
									if target_bool:
										...
										if self.save_model_after_improvement:
											...
											# if self.use_elmo:
											# 	if self.elmovocab.is_training:
											# 		self.elmovocab.modelSave(sess,os.path.join(self.save_dir,'elmo'),self.global_step)
											if self.use_albert:
												if self.albertvocab.is_training:
													self.albertvocab.modelSave(sess,os.path.join(self.save_dir,'albert'),self.global_step)
											if self.use_xlnet:
												if self.xlnetvocab.is_training:
													self.xlnetvocab.modelSave(sess,os.path.join(self.save_dir,'xlnet'),self.global_step)
										if self.parse_devset:
											self.parse_files(devset, dev_outputs, sess, print_time=False)
									else:
										steps_since_best += self.print_every
	...
	def parse(self, conllu_files, output_dir=None, output_filename=None, testing=False, debug=False,nornn=False,check_iter=False, gen_tree=False, get_argmax=False):
		...
		if compare_two:
			...
			# 	elmo_variables=set()
			if self.use_albert:
				albert_variables=set(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope='bert')) # albert use "bert" as the default scope name
			else:
				albert_variables=set()
			if self.use_xlnet:
				xlnet_variables=set(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope='model'))
			else:
				xlnet_variables=set()
			save_variables = all_variables - non_save_variables - bert_variables - albert_variables - xlnet_variables #  - elmo_variables
			saver = tf.train.Saver(list(save_variables), max_to_keep=1)
		if testing:
			enablePrint()
		config = tf.ConfigProto()
		...
		with tf.Session(config=config) as sess:
			...
			# if self.use_elmo and not self.pretrained_elmo:
			# 	if self.elmovocab.is_training:
			# 		self.elmovocab.modelRestore(sess,list(elmo_variables),model_dir=os.path.join(self.model_dir,'elmo'))
			# 	else:
			# 		self.elmovocab.modelRestore(sess,list(elmo_variables))
			if self.use_albert and not self.pretrained_albert:
				if self.albertvocab.is_training:
					self.albertvocab.modelRestore(sess,list(albert_variables),model_dir=os.path.join(self.model_dir,'albert'))
				else:
					self.albertvocab.modelRestore(sess,list(albert_variables))
			if self.use_xlnet and not self.pretrained_xlnet:
				if self.xlnetvocab.is_training:
					self.xlnetvocab.modelRestore(sess,list(xlnet_variables),model_dir=os.path.join(self.model_dir,'xlnet'))
				else:
					self.xlnetvocab.modelRestore(sess,list(xlnet_variables))
			...
			if testing:
				if not gen_tree:
					...
					for output_vocab_classname in ['DepheadIndexVocab','DeprelTokenVocab']:
						VocabClass = getattr(vocabs, output_vocab_classname)
						vocab = VocabClass(config=self._config, is_eval=is_eval)
						vocab.load() or vocab.count(self.train_conllus)
	...
	def parse_score(self, dataset, graph_outputs, sess, output_dir=None, output_filename=None, print_time=True, debug=False, check_iter=False):
		...
		return
	#=============================================================
	def parse_MST(self, dataset, graph_outputs, sess, output_dir=None, output_filename=None, print_time=True):
		""""""
		...
		return
	#=============================================================
	def parse_file(self, dataset, graph_outputs, sess, output_dir=None, output_filename=None, print_time=True,get_argmax=False):
		""""""
		...
		#if print_time:
		print('Infer time:')
		print('Parsing 1 file sents {} took {:0.1f} seconds'.format(n_sents, time.time() - graph_outputs.time))
		return
	...
	def parse_files(self, dataset, graph_outputs, sess, output_dir=None, print_time=True):
		...
		print('Parsing {} file{} sents {} took {:0.1f} seconds'.format(n_files, 's' if n_files > 1 else '', n_sents, time.time() - graph_outputs.time))
		return
