...
class TaggerNetwork(BaseNetwork):
  ...
  def build_graph(self, input_network_outputs={}, reuse=True, debug=False, nornn=False):
    ...
    with tf.variable_scope('Classifiers'):
      ...
      if 'predicate' in output_vocabs:
        vocab = output_vocabs['predicate']
        outputs[vocab.field] = vocab.get_linear_classifier(
          layer, token_weights,
          last_output if self.share_layer else None, 
          reuse=reuse)
        self._evals.add('predicate')
        if last_output is None:
          last_output = outputs[vocab.field]
      if 'psense' in output_vocabs:
        vocab = output_vocabs['psense']
        outputs[vocab.field] = vocab.get_linear_classifier(
          layer, token_weights,
          last_output if self.share_layer else None, 
          reuse=reuse)
        self._evals.add('psense')
        if last_output is None:
          last_output = outputs[vocab.field]
