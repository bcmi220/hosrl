...
class BertVocab(SetVocab):
  ...
  #=============================================================
  def __init__(self, Bert_file=None, name=None, config=None, is_eval=False):
    """"""
    ...
    super(BertVocab, self).__init__(config=config, is_eval=is_eval)
  ...
  def start_bert(self, is_eval=False):
    ...
    print('cased:', self.cased, 'tokenize_chinese_chars:', self.tokenize_chinese_chars)
    self.tokenizer=tokenization.FullTokenizer(bert_vocab_file, do_lower_case=not self.cased, tokenize_chinese_chars=self.tokenize_chinese_chars)
    ...
    print('BERT Training:', self.is_training and not is_eval, '(is_training:', self.is_training, ',', ' is_eval:', is_eval, ')')
    ...
    # if self.is_training and not is_eval:
  ...
  def get_input_tensor(self, embed_keep_prob=None, variable_scope=None, reuse=True):
    ...
    if self.pretrained:
      ...
      if self.layer_index != -1:
        outputs=self.bert_model.get_all_encoder_layers()[self.layer_index]
      else:
  ...
  def tokenize_chinese_chars(self):
    try:
      return self._config.getboolean(self, 'tokenize_chinese_chars')
    except:
      return True
  @property
