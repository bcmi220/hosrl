...
class GraphSecondTokenVocab(SecondOrderTokenVocab):
  ...
  def get_bilinear_classifier(self, layer, outputs, token_weights, variable_scope=None, reuse=False, debug=False):
    ...
    with tf.variable_scope(variable_scope or self.field):
      for i in six.moves.range(0, self.n_layers-1):
        with tf.variable_scope('FC-%d' % i):
          layer = classifiers.hidden(layer, 3*self.hidden_size,
                                      hidden_func=self.hidden_func,
                                      hidden_keep_prob=hidden_keep_prob)
      with tf.variable_scope('FC-top'):
        layers = classifiers.hiddens(layer, 3*[self.hidden_size],
                                    hidden_func=self.hidden_func,
                                    hidden_keep_prob=hidden_keep_prob)
        ...
        if self.diagonal:
          ...
          if self.use_sib:
            with tf.variable_scope('Sibling'):
              ...
              layer_sib = classifiers.trilinear_classifier(
      ...
      unary=tf.transpose(unary,perm=[0,2,1,3])
      ...
      for i in range(int(self.num_iteration)):
        ...
        #(n x o x i x ma x mb)-> (n x o x ma x mb)
        q_value=unary_potential - tf.reduce_sum(second_temp,2)
    ...
    if debug:
      ...
      outputs['labelprint']['unary']=unary
      #outputs['printdata']['binary']=binary
      outputs['labelprint']['second_temp']=second_temp
      if self.use_sib:
        outputs['labelprint']['second_temp_sib']=second_temp_sib
        outputs['labelprint']['layer_sib']=layer_sib
      if self.use_gp:
      ...
      outputs['labelprint']['layer1']=layer1
      outputs['labelprint']['layer2']=layer2
      outputs['labelprint']['layer3']=layer3
