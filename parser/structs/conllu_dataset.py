...
class CoNLLUDataset(set):
		...
		for vocab in self:
			if 'Elmo' in vocab.classname:
				if vocab.get_pretrained==True:
					self.use_elmo=True
					self.Elmo_data=h5.File(vocab.get_pretrained_elmo_path,'r')
					print('Load pretrained weight from', vocab.get_pretrained_elmo_path)
			if 'Bert' in vocab.classname:
				if vocab.get_pretrained==True:
					self.use_bert=True
					self.Bert_data=h5.File(vocab.get_pretrained_bert_path,'r')
					print('Load pretrained weight from', vocab.get_pretrained_bert_path)
			if 'Albert' in vocab.classname:
				if vocab.get_pretrained==True:
					self.use_albert=True
					self.Albert_data=h5.File(vocab.get_pretrained_albert_path,'r')
					print('Load pretrained weight from', vocab.get_pretrained_albert_path)
			if 'Xlnet' in vocab.classname:
				if vocab.get_pretrained==True:
					self.use_xlnet=True
					self.Xlnet_data=h5.File(vocab.get_pretrained_xlnet_path,'r')
					print('Load pretrained weight from', vocab.get_pretrained_xlnet_path)
			...
			if not 'seq2seq' in vocab.classname.lower():
				if hasattr(vocab,'conllu_idx'):
					...
					if vocab.classname=='DepheadBertVocab':
						tokens.insert(0,'0')
					elif vocab.classname=='DepheadElmoVocab':
						tokens.insert(0,'0')
					elif vocab.classname=='DepheadAlbertVocab':
						tokens.insert(0,'0')
					elif vocab.classname=='DepheadXlnetVocab':
						tokens.insert(0,'0')
					else:
				...
				if self.use_elmo and 'Elmo' in vocab.classname:
					...
					if sent!=[]:
						...
						indices=[]
				elif self.use_albert and 'Albert' in vocab.classname:
					if sent!=[]:
						#pdb.set_trace()
						indices=self.Albert_data[self.id_buff[-1]][...]
						# try:
						assert indices.shape[0]-1==len(sent), 'inconsistent of data size and sentence length!'
						# except:
							# pdb.set_trace()
						...
						indices=[]
				elif self.use_xlnet and 'Xlnet' in vocab.classname:
					if sent!=[]:
						#pdb.set_trace()
						indices=self.Xlnet_data[self.id_buff[-1]][...]
						# try:
						assert indices.shape[0]-1==len(sent), 'inconsistent of data size and sentence length!'
						# except:
							# pdb.set_trace()
	...
	def itersents(self,conllu_file):
		...
		if conllu_file.endswith('.zip'):
			...
			open_func = open
			kwargs = {'errors': 'ignore'}
		prefix = os.path.basename(conllu_file)
		# HACK, for predict mode
		if prefix.endswith('.end2end.input'):
			prefix = prefix.replace('.end2end.input', '.conllu')
		if prefix.endswith('.predgiven.input'):
			prefix = prefix.replace('.predgiven.input', '.conllu')
		buff_idx = 0
		with open(conllu_file,encoding='utf8') as f:
			...
			for line in reader:
				...
				if line and not line.startswith('#'):
					...
					self.id_buff.append('%s-%d'%(prefix, buff_idx))
					buff_idx += 1
					yield buff
					buff = []
			self.id_buff.append('%s-%d'%(prefix, buff_idx))
